...
def lock_door(status,prevT):
	if status == True and time.time() - prevT > 5:
		GPIO.output(RELAY,GPIO.HIGH)
		print("door lock")
	os.system("aplay /home/pi/dong.wav")
	return False
...
while True:
    ...
    if not face_detected and rects:
        face_detected = True
        start_time    = time()
		print(start_time)
    elif not rects:
...
#Take a picture to send in the email
	...
	# 	print("door lock")
	# 	os.system("aplay /home/pi/dong.wav")
	doorUnlock = lock_door(doorUnlock,prevTime)
